...
class Process:
    ...
    def process_row(self,row):
        ...
        rec["説明"] =row["説明"]
        rec["備考"] =row["備考"]
        rec["商品・通貨"] ="CURRENCY::JPY"
        rec["摘要"] =row["摘要"]
        rec["勘定科目フルネーム"] = row["貸方勘定科目フルネーム"]
        rec["勘定科目名"] = row["貸方勘定科目フルネーム"].split(":")[-1]
        rec["記号付き金額"] =f"\\{ row['金額数値']}"
        rec["金額数値"] = row["金額数値"]
        rec["照合"]="清"
        rec["レート/金額"] =1
        ...
        #借方データ
        rec =common.gnucash_empty_row()
        rec["勘定科目フルネーム"]=row["借方勘定科目フルネーム"]
        rec["勘定科目名"]=row["借方勘定科目フルネーム"].split(":")[-1]
        rec["記号付き金額"]=f"-\\{row['金額数値']}"
        rec["金額数値"]=f"-{row['金額数値']}"
        rec["照合"]="清"
        rec["レート/金額"]=1
